...
Generated by the `engineering-ruleset-plugin` sync — re-run the sync to change
it, since local edits are overwritten.

**The branch a PR merged _into_ selects the ClickUp status.** One workflow covers
every environment; the target branch is the switch:

| Merged into | ClickUp status |
|-------------|----------------|
| `dev`       | in progress    |
| `test`      | QA             |
| `prod`      | Done           |
...
fall back to their own title-cased form (`staging` → `Staging`, `perf` → `Perf`).
...
- **The status column must already exist in ClickUp.** ClickUp's API cannot
  ...
  it. So the job resolves the wanted status against the ones the task's List
  actually has and fails naming the exact column to add, rather than writing
  blind and getting an unactionable `400 Status not found`.
